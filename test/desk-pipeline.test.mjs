...
test('데스크: 응대 시작→발화 확정→호스트 급이탈 시 응대가 아카이브된다(C1) + start 난사 차단(C2)', { timeout: 60000 }, async (t) => {
  ...
  await host.ready;
  // 실제 손님 뷰어처럼 공개 디렉터리에서 접근 토큰을 받아 접속(뷰어 라우트는 토큰 필수)
  const dl = await fetch(`${srv.base}/api/desk-sessions`).then((r) => r.json());
  const tok = dl.find((x) => x.id === sess.id)?.t;
  assert.ok(tok, '데스크 세션 토큰 취득 실패: ' + JSON.stringify(dl));
  const viewer = wsClient(`${wsBase}/ws/viewer?session=${sess.id}&t=${tok}`);
  await viewer.ready;
...
test('뷰어 접근 토큰: 토큰 없거나 틀리면 /ws/viewer·/api/sessions/:id 가 거부된다', { timeout: 60000 }, async (t) => {
  const fake = await startFakeSoniox();
  const srv = await bootServer({ SONIOX_WS_URL: fake.url });
  t.after(async () => { await fake.close(); await srv.stop(); });
  const cookie = await loginAdmin(srv.base);
  const auth = { headers: { 'Content-Type': 'application/json', Cookie: cookie } };
  const sess = await fetch(`${srv.base}/api/sessions`, { method: 'POST', ...auth, body: JSON.stringify({ title: 'T', pipeline: 'soniox', preset: 'twoway', inLang: 'auto' }) }).then((r) => r.json());

  // REST: 토큰 없음 → 403, 로그인 쿠키 → 200
  const noTok = await fetch(`${srv.base}/api/sessions/${sess.id}`);
  assert.equal(noTok.status, 403, '토큰 없는 공개 세션 조회는 403 이어야 함');
  const badTok = await fetch(`${srv.base}/api/sessions/${sess.id}?t=deadbeef`);
  assert.equal(badTok.status, 403, '틀린 토큰은 403 이어야 함');
  const authed = await fetch(`${srv.base}/api/sessions/${sess.id}`, { headers: { Cookie: cookie } });
  assert.equal(authed.status, 200, '로그인 사용자는 토큰 없이 통과해야 함');

  // WS: 토큰 없음 → 연결 거부(에러)
  const wsBase = srv.base.replace('http', 'ws');
  const bad = new WebSocket(`${wsBase}/ws/viewer?session=${sess.id}`);
  await assert.rejects(new Promise((res, rej) => { bad.once('open', res); bad.once('error', rej); }), /403|Unexpected server response/, '토큰 없는 뷰어 WS 는 거부돼야 함');
});
