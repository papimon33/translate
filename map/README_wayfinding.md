...
- **문(통로) 개방**: 지정 위치(`DOORS` 상수)의 보안↔일반 경계벽을 **원형으로 일부 개방**해 '문'처럼 통로를 냄.
  현재 **3F 출국장 입구**. `{x,y,r}`(평면좌표·개방 반경)로 같은 형식 추가 가능(예: 1F 도착장 입구).
- **경로는 무조건 에스컬레이터로만 이동**(엘리베이터 제외 — `ESC_GROUPS`). 데이터상 1F~4F가 ESC로 모두 연결되어
...
검증: 1F→4F(=1F→2F→3F→4F, 전 구간 에스컬레이터·직선 리저), 2F→3F, 동일층 경로 / 게이트 마커 없음 / 상단벽 제거 / 3F 출국장 입구 문 개방 / 외부요청 0 / 콘솔 에러 없음.
