...
- **게이트/입구(통로) 표식**: 원본 도면 Entrance 좌표(`GATES` 상수)에 **항상 표시**. 글씨 없이
  문턱 바 + 바깥 더블 셰브론(청록 `COL.gate`)으로 통로 위치·방향만. (1F 게이트1~3·의전주차장출구·도착장입구 / 2F 게이트1~4 / 3F 탑승구·출국장입구 / 4F 없음)
- 경로 = **글로우 코멧 헤드 + 페이드인 잔상 점선**이 출발→목적지로 흐르는 애니메이션(밝은 파랑) + 은은한 전체 가이드. 꺾임 최소화(`reduceBends`), **층간은 수직 상승**.
