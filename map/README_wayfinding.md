...
- 각 층 도면 SVG 래스터화로 **재구성**: 바닥 footprint를 아래로 압출(슬래브 두께). **벽은 보호구역(하늘)↔일반(흰) 경계에만** 세움(그 외 벽 없음).
...
- 경로 = **밝은 파랑 동그란 점선**(애니메이션 점 없음), 꺾임 최소화(`reduceBends`), **층간은 수직 상승**.
  길찾기 차단 = 건물 밖 + 보안구역만(내부 벽 무시 → 단순/직선적). 안내데스크 동서남북 이격은 작게(`SIDE_OFF`).
