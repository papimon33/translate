...
/* ---------- 조절 상수 ---------- */
const DEFAULTS = {
  wallHeight: 16,        // 내부 벽 압출 높이(논리 px)
...
function create(opts){
  ...
  async function rasterFloor(fk){
    ...
    // 1차 판정
    const inside=new Uint8Array(mw*mh),wallM=new Uint8Array(mw*mh),secM=new Uint8Array(mw*mh);
    for(let i=0;i<mw*mh;i++){const o=i*4;if(px[o+3]<40)continue;inside[i]=1;
      const r=px[o],g=px[o+1],b=px[o+2];
      if(lumOf(r,g,b)<60)wallM[i]=1;else if(isSec(r,g,b))secM[i]=1;}
    // 외곽 벽 제거: 건물 경계(바깥) R px 이내 벽은 제외 → 내부 벽만 유지
    const R=O.wallEdgeErode,keep=new Uint8Array(mw*mh);
    for(let y=0;y<mh;y++)for(let x=0;x<mw;x++){const i=y*mw+x;if(!wallM[i])continue;
      let outer=false;
      for(let dy=-R;dy<=R&&!outer;dy++)for(let dx=-R;dx<=R;dx++){
        const nx=x+dx,ny=y+dy;
        if(nx<0||ny<0||nx>=mw||ny>=mh||!inside[ny*mw+nx]){outer=true;break;}}
      if(!outer)keep[i]=1;}
    // 마스크 채우기
    ...
    for(let i=0;i<mw*mh;i++){if(!inside[i])continue;const o=i*4;
      ...
      if(keep[i]){put(wt.data,o,cWt);put(ws.data,o,cWs);put(top.data,o,cTop);}
      else if(secM[i])put(top.data,o,cSec);
      else put(top.data,o,cTop);}
    const masks={};
  ...
  function computeRoute(deskFloor,deskSide,dest){
    ...
    for(let i=0;i<seq.length-1;i++){const Aa=seq[i],Bb=seq[i+1];const cands=PAIRG[Aa+'>'+Bb]||[];
      ...
      legs.push({floor:Aa,pts:simplifyOrtho(bp),desk:(i===0?desk:null),exitTo:{node:bnA,nodeB:bnB,transit:bg,toFloor:Bb}});
      cur=[bnB.x,bnB.y];curFloor=Bb;}
    const p=astar(FL[curFloor].grid,cur[0],cur[1],dest.x,dest.y);
    if(!p)return{error:'목적지 경로 없음'};
    legs.push({floor:curFloor,pts:simplifyOrtho(p),desk:(seq.length===1?desk:null),arrive:dest});
  ...
  function drawEscalator(lowF,nodeA,upF,nodeB){
    const p0=project(lowF,nodeA.x,nodeA.y),p1=project(upF,nodeB.x,nodeB.y);
    const vx=p1[0]-p0[0],vy=p1[1]-p0[1],L=Math.hypot(vx,vy)||1,HW=15;
    const ux=vx/L,uy=vy/L,nx=-uy*HW,ny=ux*HW;
    const a=[p0[0]+nx,p0[1]+ny],b=[p0[0]-nx,p0[1]-ny],c=[p1[0]-nx,p1[1]-ny],d=[p1[0]+nx,p1[1]+ny];
    const TH=7; // 측면 두께(아래로)
    // 측면(아래)
    svg.appendChild(poly([b,c,[c[0],c[1]+TH],[b[0],b[1]+TH]],COL.escSide,'#7f8893',0.5));
    svg.appendChild(poly([a,b,[b[0],b[1]+TH],[a[0],a[1]+TH]],COL.escSide,'#7f8893',0.5));
    // 윗면(경사)
    svg.appendChild(poly([a,b,c,d],COL.escTop,'#8f98a2',0.6));
    // 계단 줄무늬
    const N=Math.max(4,Math.round(L/14));
    for(let k=1;k<N;k++){const t=k/N,e1=lerp(a,d,t),e2=lerp(b,c,t);
      svg.appendChild(E('line',{x1:e1[0].toFixed(1),y1:e1[1].toFixed(1),x2:e2[0].toFixed(1),y2:e2[1].toFixed(1),stroke:COL.escStep,'stroke-width':0.8,'stroke-opacity':0.7}));}
    // 난간(양 옆, 살짝 띄움)
    svg.appendChild(E('line',{x1:a[0],y1:a[1]-3,x2:d[0],y2:d[1]-3,stroke:'#8b949e','stroke-width':2,'stroke-linecap':'round'}));
    svg.appendChild(E('line',{x1:b[0],y1:b[1]-3,x2:c[0],y2:c[1]-3,stroke:'#8b949e','stroke-width':2,'stroke-linecap':'round'}));
    return {p0,p1};
  }
  function drawElevator(lowF,nodeA,upF,nodeB){
    const p0=project(lowF,nodeA.x,nodeA.y),p1=project(upF,nodeB.x,nodeB.y);
    const vx=p1[0]-p0[0],vy=p1[1]-p0[1],L=Math.hypot(vx,vy)||1,HW=12;
    const ux=vx/L,uy=vy/L,nx=-uy*HW,ny=ux*HW;
    const a=[p0[0]+nx,p0[1]+ny],b=[p0[0]-nx,p0[1]-ny],c=[p1[0]-nx,p1[1]-ny],d=[p1[0]+nx,p1[1]+ny];
    svg.appendChild(poly([a,b,c,d],'#dfe5ea','#8f98a2',0.6));
    // 케이블/칸 표시
    const mid=lerp(p0,p1,0.5);
    svg.appendChild(E('line',{x1:p0[0],y1:p0[1],x2:p1[0],y2:p1[1],stroke:'#aab2bb','stroke-width':1,'stroke-dasharray':'4 4'}));
    return {p0,p1};
  }
  ...
  function drawRoute(r){
    ...
    r.legs.forEach((leg)=>{
      ...
      svg.appendChild(E('path',{d:dStr,fill:'none',stroke:COL.routeGlow,'stroke-width':12,'stroke-linecap':'round','stroke-linejoin':'round'}));
      ...
      if(leg.exitTo){
        ...
        const ep=isElev?drawElevator(leg.floor,leg.exitTo.node,leg.exitTo.toFloor,leg.exitTo.nodeB)
                       :drawEscalator(leg.floor,leg.exitTo.node,leg.exitTo.toFloor,leg.exitTo.nodeB);
        allPts.push(ep.p0,ep.p1);
        // 경유 아이콘(래퍼 없음): 에스컬=11, 엘베=12
        const mid=lerp(ep.p0,ep.p1,0.5);
        iconOnly(mid[0],mid[1],iconURL(isElev?'12':'11'),26);
      }
    });
    // 출발/목적지 = 현위치 핀
    const dl=r.legs[0];if(dl.desk){const d=project(dl.floor,dl.desk[0],dl.desk[1]);locPin(d[0],d[1],COL.start,'출발');}
    const last=r.legs[r.legs.length-1],dst=last.arrive;
    if(dst){const e=project(last.floor,dst.x,dst.y);locPin(e[0],e[1],COL.dest,dst.name);}
