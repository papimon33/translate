...
function create(opts){
  ...
  async function rasterFloor(fk){
    ...
    // 1차 판정
    // inside / 보안구역(하늘색) 판정. 도면의 검정(#000) 벽은 무시.
    const inside=new Uint8Array(mw*mh),secM=new Uint8Array(mw*mh);
    for(let i=0;i<mw*mh;i++){const o=i*4;if(px[o+3]<40)continue;inside[i]=1;
      if(isSec(px[o],px[o+1],px[o+2]))secM[i]=1;}
    // 벽 = 보호구역(하늘) ↔ 일반구역(흰)의 경계에만. 1px 경계 → R 두께로 확장. 그 외 구조물 없음.
    ...
    const R=3, wallB=new Uint8Array(mw*mh);   // 경계벽 두께(half)
    for(let y=0;y<mh;y++)for(let x=0;x<mw;x++){if(!edge[y*mw+x])continue;
      for(let dy=-R;dy<=R;dy++)for(let dx=-R;dx<=R;dx++){const nx=x+dx,ny=y+dy;
        if(nx>=0&&ny>=0&&nx<mw&&ny<mh&&inside[ny*mw+nx])wallB[ny*mw+nx]=1;}}
