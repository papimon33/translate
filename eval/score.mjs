 ...
 *   node eval/score.mjs eval/records.json --auto          # scenario_id 없는 항목을 정답 원문 유사도로 자동 매칭
 ...
 *
 * --auto: 데스크 응대 로그에서 내려받은 '평가 JSON'(관리자 → 로그 → 응대 상세)은 scenario_id 가
 *   없어도 된다 — 각 발화의 인식 원문을 30개 시나리오의 정답 원문과 CER 로 대조해 가장 가까운
 *   시나리오에 자동 배정한다(유사도 미달이면 unmatched 로 제외·표시).
 */
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { scoreAll, cer, LANG_NAME, CER_OK } from './score_core.mjs';
...
if (!recPath) { console.error('사용법: node eval/score.mjs <records.json> [--dry] [--auto] [--out result.json]'); process.exit(1); }
...
let records = Array.isArray(rec.records) ? rec.records : rec;

/* --auto: scenario_id 미지정 항목을 정답 원문 유사도(CER)로 자동 매칭.
   질문(q)=stt ↔ question.src[lang], 답변(a)=stt ↔ answer.ko_src. 0.55 초과면 미매칭 제외. */
const AUTO_MAX_CER = 0.55;
if (AUTO) {
  const unmatched = [];
  records = records.map((r) => {
    if (r.scenario_id) return r;
    let best = null, bestCer = Infinity;
    for (const sc of dataset.scenarios) {
      const gold = r.direction === 'a' ? sc.answer.ko_src : sc.question.src[r.lang];
      if (!gold) continue;
      const c = cer(gold, r.stt);
      if (c < bestCer) { bestCer = c; best = sc.id; }
    }
    if (best && bestCer <= AUTO_MAX_CER) return { ...r, scenario_id: best, auto_cer: +bestCer.toFixed(3) };
    unmatched.push(r);
    return null;
  }).filter(Boolean);
  console.log(`[auto] ${records.length}건 매칭` + (unmatched.length ? `, ${unmatched.length}건 미매칭(대본 외 발화로 제외)` : ''));
  for (const u of unmatched.slice(0, 6)) console.log('  · 미매칭:', String(u.stt || '').slice(0, 60));
}
