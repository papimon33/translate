...
## 작업 시작 전 필수
- **먼저 `PROJECT_NOTES.md`를 읽어라.** 아키텍처·두 파이프라인(whisper/translate)·핵심 로직·표시 규칙·파일 위치·WS 프로토콜·남은 배포 작업이 정리돼 있다.
...
- whisper=한·영·일·중 4개국어 동시 번역, translate=단일+띄어쓰기만 교정. 다듬기는 항상 on(토글 없음).
