...
import { kstDay, CONFFIX, confFixReal, confFixTrigger, confFixMark, isAckOnly, stripFillers, sanitizeTranslation, deskCtlAllowed } from './pipeline_util.js';

// 엔진 접속 URL — 테스트에서 로컬 fake 엔진으로 오버라이드(실 엔진 호출은 항상 과금되므로)
const SONIOX_WS_URL = process.env.SONIOX_WS_URL || 'wss://stt-rt.soniox.com/transcribe-websocket';
const CARTESIA_TTS_WS_BASE = process.env.CARTESIA_TTS_WS_URL || 'wss://api.cartesia.ai/tts/websocket';
...
/* ================================================================== */
const DATA_DIR = process.env.DATA_DIR ? path.resolve(process.env.DATA_DIR) : path.join(__dirname, 'data'); // 테스트는 임시 디렉터리로 격리
...
let vendorUsage = { soniox: {}, cartesia: {}, openai: {}, cartesiaSelf: {} }; // cartesiaSelf: 파이프라인별 자체 집계(벤더 API 는 태그 미지원)
...
async function loadStore() {
  if (MONGODB_URI) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (vu) vendorUsage = { soniox: vu.soniox || {}, cartesia: vu.cartesia || {}, openai: vu.openai || {}, cartesiaSelf: vu.cartesiaSelf || {} };
        ...
        col = usersCol = usageCol = usageHourlyCol = termsConfigCol = summariesCol = vendorUsageCol = cannedCol = deskRegistryCol = null;
  ...
  loadFile(VENDOR_USAGE_FILE, (v) => { if (v && typeof v === 'object') vendorUsage = { soniox: v.soniox || {}, cartesia: v.cartesia || {}, openai: v.openai || {}, cartesiaSelf: v.cartesiaSelf || {} }; });
...
// Cartesia 파이프라인별 자체 집계 — 합성 요청 시 문자수·건수 누적(desk=안내데스크, live=실시간 번역)
let ttsUsageTimer = null;
function recordTts(pipe, chars) {
  try {
    const day = kstDay(Date.now());
    const d = vendorUsage.cartesiaSelf[day] || (vendorUsage.cartesiaSelf[day] = {});
    const b = d[pipe] || (d[pipe] = { chars: 0, req: 0 });
    b.chars += chars; b.req += 1;
    if (!ttsUsageTimer) ttsUsageTimer = setTimeout(() => { ttsUsageTimer = null; persistVendorUsage().catch(() => {}); }, 5000);
  } catch {}
}
...
/* ---- 데스크 현황: 전 데스크의 실시간 상태(호스트 연결·응대 중·뷰어 수) — 목록 화면 현황판용 ---- */
app.get('/api/desk-status', requireAuth, (req, res) => {
  const out = sessions
    .filter((s) => s.pipeline === 'desk' && !s.deletedAt)
    .map((s) => {
      const r = rooms.get(s.id);
      const dc = deskCtrl.get(s.id);
      const st = dc && dc.state ? dc.state() : null;
      return {
        id: s.id,
...
/* ---- 전체 초기화(관리자): 세션(대화·응대 로그 포함)·AI 요약을 전부 삭제 ----
   파괴적 작업이므로 로그인 세션과 별개로 관리자 비밀번호를 본문에서 재확인한다.
   보존: 계정·안내데스크 정의·용어·정형 안내·사용량 통계(과금 기록). */
app.post('/api/admin/reset-all', requireAdmin, async (req, res) => {
  const pw = String((req.body || {}).password || '');
  const a = Buffer.from(pw), e = Buffer.from(ADMIN_PASSWORD);
  if (!(a.length === e.length && crypto.timingSafeEqual(a, e))) {
    return res.status(403).json({ error: '관리자 비밀번호가 일치하지 않습니다.' });
  }
  // 진행 중인 캡처·뷰어 연결을 먼저 정리(파이프라인 close 핸들러가 정상 경로로 정돈)
  ...
  summaries = [];
  try {
    if (col) { if (sessionsFlushInflight) await sessionsFlushInflight; await col.deleteMany({}); }
    else await flushSessions();
    if (summariesCol) await summariesCol.deleteMany({});
    else writeDataFile(SUMMARIES_FILE, '[]');
  } catch (err) {
    console.error('[reset-all] 저장소 정리 실패', err);
    return res.status(500).json({ error: '저장소 정리에 실패했습니다 — 서버 로그를 확인하세요.' });
  }
  console.warn(`[reset-all] 관리자 초기화 — 세션 ${n}개(응대 로그 포함)·요약 삭제`);
  res.json({ ok: true, removedSessions: n });
});

/* ---- 전체 데이터 백업(관리자): 세션·데스크 정의·용어·정형 안내를 JSON 한 파일로 ---- */
app.get('/api/admin/export', requireAdmin, (req, res) => {
  res.setHeader('Content-Type', 'application/json');
  res.setHeader('Content-Disposition', `attachment; filename="airtalk-backup-${new Date().toISOString().slice(0, 10)}.json"`);
  res.send(JSON.stringify({ exportedAt: Date.now(), sessions, deskRegistry, termsConfig, canned: cannedConfig }, null, 1));
});
...
async function fetchSonioxRange(startMs, endMs) {
  ...
  for (let page = 0; page < 40; page++) { // 안전 상한(최대 40,000건)
    ...
    for (const e of d.usage_logs || []) {
      ...
      const b = byDay[day] || (byDay[day] = { audioMs: 0, costUsd: 0, requests: 0, byUser: {}, byPipe: {} });
      const ms = Number(e.input_audio_duration_ms) || 0;
      const cost = Number(e.cost_usd) || 0;
      b.audioMs += ms; b.costUsd += cost; b.requests += 1;
      // client_reference_id('u:<id>|p:<desk|live>') → 유저별·파이프라인별 집계 (미태깅 과거 요청은 anon/etc)
      const ref = String(e.client_reference_id || '');
      const um = ref.match(/^u:([^|]*)/);
      const uid = um ? um[1] : 'anon';
      const pm = ref.match(/\|p:(desk|live)/);
      const pipe = pm ? pm[1] : 'etc';
      const u = b.byUser[uid] || (b.byUser[uid] = { audioMs: 0, costUsd: 0, requests: 0 });
      u.audioMs += ms; u.costUsd += cost; u.requests += 1;
      const pp = b.byPipe[pipe] || (b.byPipe[pipe] = { audioMs: 0, costUsd: 0, requests: 0 });
      pp.audioMs += ms; pp.costUsd += cost; pp.requests += 1;
    }
  ...
  for (const k of Object.keys(byDay)) {
    ...
    for (const u of Object.values(byDay[k].byPipe || {})) u.costUsd = +u.costUsd.toFixed(4);
  }
  return { byDay };
}
...
async function fetchSoniox(spanDays) {
  ...
  const mergeDay = (into, m) => {
    ...
    into.byPipe = into.byPipe || {};
    for (const [pk, u] of Object.entries(m.byPipe || {})) {
      const t = into.byPipe[pk] || (into.byPipe[pk] = { audioMs: 0, costUsd: 0, requests: 0 });
      t.audioMs += u.audioMs; t.costUsd += u.costUsd; t.requests += u.requests;
    }
  };
...
app.get('/api/admin/vendor-usage', requireAdmin, async (req, res) => {
  ...
  res.json({
    ...
    soniox: build('soniox', (ds) => {
      ...
      // 파이프라인별(안내데스크/실시간 번역) 합계 — client_reference_id 의 |p: 태그 기준(구 로그는 etc)
      const byPipe = { desk: { audioMs: 0, costUsd: 0, requests: 0 }, live: { audioMs: 0, costUsd: 0, requests: 0 }, etc: { audioMs: 0, costUsd: 0, requests: 0 } };
      for (const d of ds) for (const [pk, u] of Object.entries(d.byPipe || {})) {
        const t = byPipe[pk] || byPipe.etc;
        t.audioMs += u.audioMs; t.costUsd += u.costUsd; t.requests += u.requests;
      }
      for (const t of Object.values(byPipe)) t.costUsd = +t.costUsd.toFixed(4);
      return { totalCostUsd: +ds.reduce((a, d) => a + (d.costUsd || 0), 0).toFixed(4), totalAudioMin: +(ds.reduce((a, d) => a + (d.audioMs || 0), 0) / 60000).toFixed(1), totalRequests: ds.reduce((a, d) => a + (d.requests || 0), 0), users, byPipe };
    }),
    cartesia: build('cartesia', (ds) => {
      // 파이프라인별 자체 집계(문자수 기준 — Cartesia 벤더 API 는 태그별 분해 미지원)
      const byPipe = { desk: { chars: 0, req: 0 }, live: { chars: 0, req: 0 } };
      for (const d of ds) {
        const s = vendorUsage.cartesiaSelf[d.date];
        if (!s) continue;
        for (const pk of ['desk', 'live']) { if (s[pk]) { byPipe[pk].chars += s[pk].chars || 0; byPipe[pk].req += s[pk].req || 0; } }
      }
      return { totalCredits: ds.reduce((a, d) => a + (d.credits || 0), 0), byPipe };
    }),
...
function validateSonioxPair(a, b, ctx) {
  return new Promise((resolve) => {
    ...
    let ws;
    try { ws = new WebSocket(SONIOX_WS_URL); }
...
app.post('/api/sessions', requireAuth, (req, res) => {
  const now = Date.now();
  const b = req.body || {};
  const pipeline = ['translate', 'soniox', 'desk'].includes(b.pipeline) ? b.pipeline : 'soniox'; // whisper(구) 지원 종료 — 기본 soniox
  ...
  // translate 는 단일 출력 언어. desk 는 ko 시작(감지로 동적 확장). soniox 다국어.
...
function cartesiaConnect() {
  ...
  cartesiaConnecting = new Promise((resolve, reject) => {
    ...
    let ws;
    try {
      ws = new WebSocket(`${CARTESIA_TTS_WS_BASE}?api_key=${encodeURIComponent(CARTESIA_API_KEY)}&cartesia_version=${encodeURIComponent(CARTESIA_VERSION)}`);
...
function startTalkPipeline(sessionId, side) {
  ...
  const config = {
    ...
    language_hints: [a, b], translation: { type: 'two_way', language_a: a, language_b: b },
    client_reference_id: 'u:' + ((getSession(sessionId) || {}).owner || 'anon') + '|p:live', // 유저별+파이프라인 사용량(발화자는 익명 뷰어 → 세션 소유자로 귀속)
    ...((() => { const c = buildSonioxContext([a, b]); return c ? { context: c } : {}; })()), // 고유명사/번역 설정(활성 쌍)
  };
  const sx = new WebSocket(SONIOX_WS_URL);
  ...
  const commit = () => {
    ...
    if (cfg.ttsOn) {
      ...
      recordTts('live', out.length); // 폰 PTT = 실시간 번역 계열
...
function handleHost(ws) {
  ...
  const upsertItem = (id, langTexts, source, speaker, lang, sideOv, toks, tm) => {
  ...
  function runSoniox() {
    ...
    let ttsOn = ws._tts === '1' && !!CARTESIA_API_KEY;
    ...
    const config = {
      ...
      client_reference_id: 'u:' + (ws._userId || 'anon') + '|p:live', // 유저별+파이프라인(실시간 번역) 사용량 집계
    ...
    let utterTm = null; // 진행 중 발화의 레이턴시 타임스탬프 { s, e, (tq, ta 는 발화 중 TTS 시) }
    // 레이턴시 로그: 카드(id)의 tm 에 시각 병합 — 커밋 전(카드 미생성)이면 utterTm 에 보류.
    // 첫 발생만 기록(문장 여러 개여도 '첫 오디오까지'가 체감 지연).
    ...
    const speakTts = (text, forId) => {
      ...
      ttsChain = ttsChain.then(() => {
        ...
        recordTts(pipeline === 'desk' ? 'desk' : 'live', tx.length); // 파이프라인별 자체 사용량
    ...
    const flushTtsSentences = () => {
      ...
      for (const s of out) speakTts(s, curId);
    };
    ...
    const commit = () => {
      ...
      const tm = { ...(utterTm || {}), c: Date.now() }; // 레이턴시: s(첫 토큰)/e(<end>)/c(확정)
      utterTm = null;
      ...
      if (ttsOn) {
        ttsLang = target;
        const tl = tail.trim();
        if (tl) speakTts(tl, id);                   // 종결부호 없이 남은 번역 꼬리
        else if (!hadFinal) speakTts(out, id);      // 발화 중 합성된 게 없으면(번역 미확정) 전체 합성
      }
    };
    ...
    function connectEngine() {
      ...
      const cur = new WebSocket(SONIOX_WS_URL);
      sx = cur;
      cur.on('open', () => {
      if (sx !== cur) return;
    ...
    cur.on('message', (raw) => {
      ...
      if (shownTgt) lastTrans = shownTgt;
      if (!curId && (shownSrc || shownTgt)) { curId = newId(); utterTm = { s: Date.now() }; }
      if (curId) {
      ...
      if (endHit) { if (utterTm && utterTm.e == null) utterTm.e = Date.now(); commit(); }
    });
  ...
  function runDesk() {
    ...
    let staffTm = null, guestTm = null; // 채널별 발화 레이턴시 타임스탬프 { s, e } (커밋 시 c 추가)
    ...
    const baseConfig = (langs) => ({
      api_key: SONIOX_API_KEY, model: 'stt-rt-v5', audio_format: 'pcm_s16le', sample_rate: 24000, num_channels: 1,
      client_reference_id: 'u:' + (ws._userId || 'anon') + '|p:desk', // 유저별+파이프라인(안내데스크) 사용량 집계
      enable_language_identification: true, enable_endpoint_detection: true,
      endpoint_sensitivity: Number.isFinite(sens) ? Math.min(1, Math.max(-1, sens)) : 0,
      max_endpoint_delay_ms: Number.isFinite(maxDelay) ? Math.min(3000, Math.max(500, maxDelay)) : 2000,
      endpoint_latency_adjustment_level: Number.isFinite(latency) ? Math.min(3, Math.max(0, Math.round(latency))) : 0,
    ...
    function connectSx() {
      ...
      const old = sx;
      sxReady = false;
      const next = new WebSocket(SONIOX_WS_URL);
      sx = next;
      next.on('open', () => {
    ...
    function onSxMessage(raw) {
      ...
      const shownTgt = (finalTrans + nonFinalTrans).trim();
      if (shownTgt) lastTrans = shownTgt;
      if (!curId && (shownSrc || shownTgt)) { curId = newId(); staffTm = { s: Date.now() }; } // 레이턴시: 발화 시작
      ...
      if (endHit) { if (staffTm && staffTm.e == null) staffTm.e = Date.now(); commit(); } // 길이 기반 강제 확정 제거
    }
    ...
    function onGuestMsg(raw) {
      ...
      if (!gCurId && (shownSrc || shownTgt)) { gCurId = newId(); guestTm = { s: Date.now() }; } // 레이턴시: 발화 시작
      ...
      if (endHit) { if (guestTm && guestTm.e == null) guestTm.e = Date.now(); guestCommit(); } // 길이 기반 강제 확정 제거
    }
    ...
    const ctrl = {
      ...
      state: () => ({ phase, lang: lockedB || null }), // 데스크 현황판용(응대 중 여부·손님 언어)
