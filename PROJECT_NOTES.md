...
- 페이지 제목(실시간 번역/데스크 안내)은 좌측 정렬. 세션 내 컨트롤 바는 한 줄: 옵션(좌) · 마이크 입력(우) · 옵션 숨김 버튼(최우측, 숨겨도 마이크는 표시).

## 데스크 안내 모드 (pipeline='desk')
- 데스크(국제선 1층/2층 등)마다 세션 1개. 호스트가 세션에 들어가 **'마이크 허용하고 대기 시작'** 버튼을 눌러야 대기 시작(자동 시작 없음). 대기 중에는 **soniox 세션을 열지 않음**(서버가 오디오 버림 — 비용 0), 60초 idle-stop 도 데스크는 제외(무기한 대기).
- 세션 내 QR('뷰어 연결') = `desk.html?session=ID`(해당 데스크 전용 뷰어). 뷰어 플로우: **입장 화면**(세션명/안내데스크명 + 입장 버튼, 동서남북 미표기) → 입장 시 전체화면 → **터치 대기 화면**("화면을 터치해서 통역을 시작하세요") → **언어 선택**(한·영·일·중, 20초 방치 시 복귀) → soniox two_way(ko↔선택언어) 시작 → 번역 텍스트 화면(stage).
- 통역 시작/종료 프로토콜: 뷰어 `{type:'desk-start', lang}` → 서버 `deskCtrl`(sessionId→start/end) 경유 호스트 파이프라인 `startConversation` → 모두에게 `{type:'desk-active', lang}`. 종료(무음 deskIdle **기본 30초**·뷰어 ✕ `desk-end`·호스트 '대기모드로' `desk-reset-now`·호스트 이탈) → items 를 deskLog 에 보존 후 `desk-reset` → 뷰어는 터치 화면, 호스트는 대기 복귀. 호스트도 하단에서 언어 고르고 '통역 시작' 가능(`audio.js deskStart(lang)`).
- 손님이 한국어 선택 시 two_way 불가 → one_way(전부 ko 표기)로 폴백. 호스트 미준비 상태에서 뷰어가 시작하면 status 안내 후 터치 화면 복귀.
- 길안내(wayfind): 통역 중 안내원(ko) 답변에서 시설 감지 → 뷰어 채팅 아래 인라인 지도. 랜딩(세션 미지정 `desk.html`)은 안내데스크 선택 목록 유지.
